...
$("#frmAddProduct").submit(function(event) {
    ...
    var $form = $(this), nombre = $form.find("input[name='nombre']").val()
            ...
            var categorias = document.getElementById("categoriaProductAdd");
    ...
    posting.done(function(data) {
        if (data !== null) {
            ...
            fila = fila + '<td class="SalsaProducto">' + ((obj.Sauce === 't') ? 'Si':'No') + '</td>';
            fila = fila + '<td class="PrecioProducto">' + obj.Price + '</td>';
            fila = fila + '<td class="CategoriaProducto" id="cateP'+obj.IdCategory+'">' + nombreCategoria + '</td>';
            fila = fila + '<td class="EstadoProducto">Activo</td>';
